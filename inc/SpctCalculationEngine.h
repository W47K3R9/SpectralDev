...
{
  public:
    CalculationEngine(std::shared_ptr<ResynthOscs<T, WAVETABLE_SIZE, FFT_SIZE>> resynth_osc_ptr,
                      std::shared_ptr<CircularSampleBuffer<T, FFT_SIZE, WAVETABLE_SIZE>> circular_sample_buffer_ptr,
        ...
        : m_resynth_oscs_ptr{std::move(resynth_osc_ptr)},
          m_circular_buffer_ptr{std::move(circular_sample_buffer_ptr)},
    ...
    void set_threshold(const float threshold) noexcept { m_threshold = threshold; }
    ...
    void set_voices(const size_t num_voices) noexcept { m_voices = std::clamp<size_t>(num_voices, 0, MAX_VOICES); }
  ...
  private:
    ...
    void fft_calculation()
    {
        ...
        while (!m_stop_workers)
        {
            ...
            if (!m_stop_workers)
            {
                ...
                auto& fft_samples = m_circular_buffer_ptr->m_out_array;
                // pass the whole array as reference to the FFT, will change the output array!
                spct_fourier_transform<T, degree_of_pow_two_value(FFT_SIZE)>(fft_samples, m_exponent_lut);
    ...
    // fft-related
    BinMagArr<T, (FFT_SIZE / 2)> m_bin_mag_arr;
    ...
    std::atomic<float> m_threshold = MIN_GAIN_THRESHOLD<float>;
