...
class BufferManager
{
  ...
  private:
    ...
    const unsigned int m_transform_interval = 100;
...
void BufferManager<T, BUFFER_SIZE>::process_daw_chunk(T* daw_chunk, const size_t t_size, const T threshold)
{
    ...
    // m_oscillators.select_waveform(OscWaveform::SAW);
    for (size_t step = 0; step < steps_needed; ++step)
    {
        ...
        if (do_transformation && (m_act_interval == m_transform_interval))
        {
        ...
        else if (do_transformation && !(m_act_interval == m_transform_interval))
        {
            ++m_act_interval;
        }
    }
