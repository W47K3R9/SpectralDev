...
static void dummy_fill(double* t_arr, const size_t t_arr_size)
{
    ...
    {
        t_arr[index] = index + 0.5; // NOLINT(*-narrowing-conversions, *-pointer-arithmetic)
    }
}
...
inline void test_buffer_manager()
{
    ...
    /// std::cout << "Base case algorithm took " << std::chrono::duration_cast<std::chrono::microseconds>(end -
    /// now).count()
    ///           << " µs." << std::endl;
    ...
    ///           << std::endl;
    /// std::cout << "Total calculation time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end -
    /// now).count()
    ///           << " ms." << std::endl;
